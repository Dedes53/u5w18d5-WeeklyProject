...
public class PrenotazioneService {
    ...
    public Prenotazione prenota(Utente u, Postazione p, LocalDate giorno) {
        ...
        if (prenotazioneRepository.existsByUtenteAndGiorno(u, giorno)) {
            throw new PrenotazioneDuplicataException("L' utente " + u.getName() + ' ' + u.getSurname() + " ha già una prenotazione per il giorno " + giorno);
        }
        ...
        if (numeroPrenotazioni >= p.getNumMax()) {
            throw new PostazionePienaException("La postazione " + p.getDescrizione() + " è già piena per il giorno " + giorno);
        }
